...
.. note::
  ...
  Unit tests for the **Universal HTTP Client** are written using `pytest`_ and
  a comprehensive set of test automation are provided by `tox`_.

There are many schools of thought when it comes to test design. When building the
**Universal HTTP Client**, we decided to focus on practicality. That means:
  ...
  * **Coverage matters...kind of.** We have documented the primary intended
    behavior of every function in the **Universal HTTP Client** library, and the
...
* ``test_validators.py`` tests validator functions found in
  ``universal_http_client/_validators.py``
...
.. tabs::
  ...
  .. tab:: Installing via pip
    ...
    .. code-block:: bash
      ...
      $ pip install universal_http_client[tests]
...
The **Universal HTTP Client** library does not use any custom command-line
options in its test suite.
...
.. tip::
  ...
  .. code-block:: bash
    ...
    universal-http-client $ cd tests/
    universal-http-client/tests/ $ pytest --help

Configuration File
===================

Because the **Universal HTTP Client** has a very simple test suite, we have not
...
.. note::
  ...
  Because of the simplicity of the **Universal HTTP Client**, the test suite does
...
.. note::
  ...
  The **Universal HTTP Client** test suite does support incremental testing using,
